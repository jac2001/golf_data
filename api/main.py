...
def _career_from_csvs(player: str) -> dict:
    """Career without DuckDB (the cloud has no golf_data.db).

    Preferred source: data/processed/player_careers.csv — a committed
    export of the exact DB join (export_player_careers.py), so SG and
    scoring columns are fully populated. Regenerated by post_tournament
    every settle. Falls back to the bare leaderboard CSVs (results only,
    SG dashed) if the export is missing."""
    player_key = _name_key(player)
            ...
            if not df.empty:
                ...
                for _, r in df.head(50).iterrows():
                    recent.append({
                        ...
                        "earnings": earn(r.get("earnings")),
                        "rounds_played": int(r["rounds_played"]) if pd.notna(r.get("rounds_played")) else None,
                        "r1": sf(r.get("r1")), "r2": sf(r.get("r2")),
                        "r3": sf(r.get("r3")), "r4": sf(r.get("r4")),
                        ...
                        "scoring_avg": sf(r.get("scoring_avg")), "birdie_pct": sf(r.get("birdie_pct")),
                    })
                ...
                for yr, g in df.groupby("year"):
                    by_year.append({
                        ...
                        "avg_sg_total": sf(g["sg_total"].mean()),
                        "avg_sg_ott": sf(g["sg_ott"].mean()),
                        "avg_sg_app": sf(g["sg_app"].mean()),
                        "avg_sg_arg": sf(g["sg_arg"].mean()),
                        "avg_sg_putt": sf(g["sg_putt"].mean()),
                        "avg_driving_dist": sf(g["driving_dist"].mean()),
                        "avg_driving_acc": sf(g["driving_acc"].mean()),
                        "avg_gir_pct": sf(g["gir_pct"].mean()),
                        "avg_scrambling": sf(g["scrambling"].mean()),
                        "avg_scoring": sf(g["scoring_avg"].mean()),
                    })
                by_year.sort(key=lambda y: -y["year"])
                return {"recent": recent, "by_year": by_year}
        except Exception:
            pass  # fall through to the bare-CSV path
    frames = []
