...
class GolfAssistant:
    ...
    def _intent_directives(self, intent: str) -> List[str]:
        ...
        mapping = {
            ...
            "player_odds": [
                ...
                "- Compare model_prob vs book implied_prob using the BETTING EDGE LOGIC rules.",
                "- If model_prob > implied_prob: call out the value and size of the edge.",
                "- If model_prob < implied_prob: say the market is overpricing this player — do NOT call it value.",
            ...
            "model_vs_odds": [
                ...
                "- VALUE plays: model_prob > book_implied_prob (we like more than the market does).",
                "- FADES: model_prob < book_implied_prob (market is overpricing, avoid).",
    ...
    def _build_system_prompt(self, intent: str = "unknown") -> str:
        ...
        prompt_parts = [
            ...
            "2) PERFORMANCE STATS: SG trends, recent form, consecutive cuts, FormTrend — this is the foundation of every answer.",
            "3) COURSE HISTORY: wins, avg finish, course-fit metrics at THIS specific venue.",
            "4) CONTEXTUAL SIGNALS (support only): power rankings, expert commentary, article blurbs/narratives.",
            "5) ODDS (only when directly asked about betting): market implied probabilities, line movement.",
            "- Odds are NEVER the lead reason in a recommendation. Always lead with what the player has actually done.",
            "- If expert/context conflicts with stats: flag conflict, reduce confidence, explain the disagreement.",
            "- If players are statistically similar, course history and form trend break the tie.",
            "",
            "## HOW TO ANSWER",
        ...
        prompt_parts.extend([
            ...
            "",
            "## STATS TRANSLATION GUIDE (use these plain-English translations)",
            "- SG/Rd (SG:Total): Strokes gained per round vs. the field. +2.0 means he plays 2 shots better per round than the average pro. ELITE = +1.5+, GOOD = +0.5 to +1.5, AVERAGE = 0 to +0.5",
            "- SG:APP (Approach): How well he hits it close to the pin. The biggest predictor of success on most courses.",
            "- SG:OTT (Off the Tee): How far and straight he drives it. More important on long, open courses.",
            "- SG:ARG (Around the Green): Chipping and short game. Critical on courses with thick rough.",
            "- SG:PUTT: How many putts he takes. Matters most on courses with fast, tricky greens.",
            "- RecentForm: SG average over last 6-8 events. Positive = in form, negative = struggling.",
            "- CutStreak: Consecutive cuts made. 5+ in a row = reliable, consistent player right now.",
            "- FormTrend: HOT = improving, WARM = solid, NEUTRAL = steady, COLD = declining.",
            "- ProjScore: Our projected score vs. field median (e.g. -4.1 means we project him 4 shots better than the average player in the field).",
            "- CourseAvg: His average finishing position at this specific course. Lower = better.",
            "",
            "## HOW TO ANSWER QUESTIONS",
            "- Lead with the performance story — recent results, form, course history FIRST",
            "- Translate every stat into plain English. Never just list numbers.",
            "- Structure every player answer as: (1) Form & recent results, (2) Course history, (3) Why the course suits his game, (4) Any concerns",
            "- End with a clear YES / NO / WAIT recommendation in one sentence",
            "",
            "## SECTION INCLUSION RULES — FOLLOW STRICTLY",
            "- BETTING ANGLE section: ONLY include if the user's question contains the words 'bet', 'betting', 'odds', 'wager', or 'sportsbook'. Otherwise omit entirely.",
            "- LEAGUE STANDING section: ONLY include if the user asks about lineup, roster, who to pick, or usage decisions. Omit for general 'is X good this week?' questions.",
            "- NEVER volunteer odds, implied probabilities, or edge calculations unless betting is explicitly asked about.",
            "- For 'is X worth using?' or 'why is X a good play?' — answer purely with performance, form, and course fit. That is the complete answer.",
            "- USAGE: You have usage remaining data as background context. Only mention it if two players are statistically very close AND the user is asking who to pick between them. Never lead with it, never use it to dismiss a player who has the better stats.",
            "",
            "## EXAMPLE GOOD RESPONSE",
            "Question: 'Is Rory McIlroy worth using this week?'",
            "Good: 'Yes — Rory is a strong play here. He has 2 wins at TPC Sawgrass and averages -8.0 under",
            "par per tournament, which is elite course history. His short game ranks top 5 in this field,",
            "and that matters here because THE PLAYERS punishes any miss around the greens. He missed the",
            "cut last week, but one bad result doesn't erase what he consistently does at this course.",
            "His form over the last 2 months is solid — he's been trending upward. The concern is whether",
            "last week was a blip or a sign of something. Given his history here specifically, I'd lean",
            "toward using him. Yes, use him.'",
            "",
            "Bad: 'McIlroy top-20 odds are -120 (45% implied). We project him at nearly 100% top-20.",
            "EV is +$0.83 per dollar. Strong betting value. League standing: you are 9th...'",
            "",
            "## BETTING EDGE LOGIC — READ CAREFULLY",
            "Value and overpriced are defined strictly by comparing model probability vs book implied probability:",
            "- model_prob > book_implied_prob → book is UNDERPRICING the player → VALUE BET (player is undervalued by market)",
            "- model_prob < book_implied_prob → book is OVERPRICING the player → FADE (player is overvalued by market)",
            "- model_prob ≈ book_implied_prob (within 3pp) → FAIR VALUE — no edge",
            "",
            "EXAMPLES:",
            "- Model: 8% win, Book implies 17% → book overprices player → FADE on outright, do NOT call this 'undervalued'",
            "- Model: 25% win, Book implies 12% → book underprices player → VALUE, player is 'undervalued by the market'",
            "- NEVER say a player is 'undervalued' or 'good value' when model_prob < book_implied_prob",
            "",
            "TOP20/TOP10/TOP5 PROBABILITY DISPLAY RULES:",
            "- Never state a top20 probability above 85% — our model is known to overestimate these markets",
            "- Never say 'nearly 100%', 'almost certain', or 'locks' for any finish position",
            "- Use language like 'very likely', 'strong candidate', 'solid top-20 profile' instead of exact numbers above 80%",
            "",
            "## CRITICAL CONSTRAINT",
    ...
    BETTING_INTENTS = {"player_odds", "odds_movement", "odds_favorites", "model_vs_odds"}
    BETTING_KEYWORDS = {"bet", "betting", "odds", "wager", "sportsbook", "prop", "parlay", "edge"}

    def _is_betting_question(self, intent: str, question: str = "") -> bool:
        """Return True only if the user is explicitly asking about betting."""
        if intent in self.BETTING_INTENTS:
            return True
        q = question.lower()
        return any(kw in q for kw in self.BETTING_KEYWORDS)

    def _build_context(self, tournament_name: Optional[str] = None, intent: str = "unknown", question: str = "") -> str:
        """Build context about current predictions and player data."""
        context_parts = []
        include_betting = self._is_betting_question(intent, question)
        ...
        if self.predictions is not None:
            ...
            context_parts.append("| Player | OWGR | SG/Rd | SG:APP | SG:OTT | RecentForm | CoursWins | CourseAvg | CutStreak | FormTrend | ProjScore | Win% | EV |")
            context_parts.append("|--------|------|-------|--------|--------|------------|-----------|-----------|-----------|-----------|-----------|------|-----|")

            for _, row in top_15.iterrows():
                name = row.get('player_name', 'Unknown')
                rank = int(row.get('world_rank', 999)) if pd.notna(row.get('world_rank')) else 'N/A'
                sg_total = row.get('sg_total', 0)
                sg_app = row.get('sg_app', 0)
                sg_ott = row.get('sg_ott', 0)
                recent_sg = row.get('recent_sg_weighted', 0)
                hist_wins = int(row.get('hist_wins', 0)) if pd.notna(row.get('hist_wins')) else 0
                hist_avg = row.get('hist_avg_finish', None)
                hist_avg_str = f"T{hist_avg:.0f}" if pd.notna(hist_avg) else "No hist"
                cuts = int(row.get('consecutive_cuts', 0)) if pd.notna(row.get('consecutive_cuts')) else 0
                form = row.get('form_trend', 'N/A')
                proj = row.get('proj_median', None)
                proj_str = f"{proj:+.1f}" if pd.notna(proj) else "N/A"
                win = row.get('win_prob_calibrated', row.get('win_prob', 0)) * 100
                ev = row.get('expected_value', 0)

                context_parts.append(
                    f"| {name} | #{rank} | {sg_total:+.2f} | {sg_app:+.2f} | {sg_ott:+.2f} | {recent_sg:+.2f} | {hist_wins}W | {hist_avg_str} | {cuts} cuts | {form} | {proj_str} | {win:.1f}% | ${ev:,.0f} |"
                )

        # Odds — slim reference table always included; full betting profiles only when betting question
        if self.betting_profiles is not None and "player_name" in self.betting_profiles.columns:
            ...
            if "odds_rank" in bp.columns:
                bp = bp[pd.notna(bp["odds_rank"])].nsmallest(20, "odds_rank")
            else:
                bp = bp.head(20)

            context_parts.append("")
            if include_betting:
                context_parts.append("## ODDS & BETTING PROFILES")
                context_parts.append("| Player | Odds | Implied% | Movement | Notes |")
                context_parts.append("|--------|------|----------|----------|-------|")
                for _, row in bp.iterrows():
                    name = row.get("player_name", "Unknown")
                    odds = row.get("odds_to_win", "")
                    prob = row.get("implied_prob", "")
                    prob = f"{float(prob):.1%}" if pd.notna(prob) else ""
                    swing = row.get("odds_swing", "")
                    notes = f"Line {str(swing).lower()}" if swing and pd.notna(swing) else ""
                    analysis = str(row.get("analysis", "") or "")[:60]
                    context_parts.append(f"| {name} | {odds} | {prob} | {notes} | {analysis} |")
            else:
                context_parts.append("## OUTRIGHT ODDS (reference only)")
                context_parts.append("| Player | Odds to Win |")
                context_parts.append("|--------|-------------|")
                for _, row in bp.iterrows():
                    name = row.get("player_name", "Unknown")
                    odds = row.get("odds_to_win", "")
                    context_parts.append(f"| {name} | {odds} |")
        ...
        # Player usage — background context only, not a decision driver
        if self.usage_tracker is not None:
            try:
                used_up = self.usage_tracker[self.usage_tracker['uses_remaining'] == 0]['player_name'].tolist()
                one_left = self.usage_tracker[self.usage_tracker['uses_remaining'] == 1]['player_name'].tolist()
                context_parts.append("")
                context_parts.append("## USAGE REMAINING (background info)")
                if used_up:
                    context_parts.append(f"No uses left: {', '.join(used_up[:10])}")
                if one_left:
                    context_parts.append(f"1 use left: {', '.join(one_left[:10])}")
                context_parts.append("(Usage is a tiebreaker only — never the primary reason to avoid a player)")
            except Exception:
                pass
